    ...
    if (state == 3 || dsid == 0) {
        if (!opus_buf_.empty()) {
            ...
            if (!wav.empty() && on_decode_) {
                ...
                info << "segments=" << segment_count_
                     << " bytes=" << opus_buf_.size()
                     << " state=" << (int)state;
...
std::string OpusAccumulator::decode_opus(const std::vector<uint8_t>& opus_data) {
    ...
    std::string opus_file = "/home/yu/workspace/uart_opus_"
                          + std::to_string(file_seq) + ".opus";
    std::string wav_file  = "/home/yu/workspace/uart_wav_"
                          + std::to_string(file_seq) + ".wav";
