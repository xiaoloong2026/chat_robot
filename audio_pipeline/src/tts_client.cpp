...
void dispatch_segment_result(uint64_t sequence, SegmentResult result) {
    ...
    while (true) {
        ...
        if (g_segment_ready_callback && !completed_mp3.empty())
            g_segment_ready_callback(ready.text, completed_mp3);
...
bool convert_wav_to_mp3(const std::string& wav_path,
    ...
    cmd << "ffmpeg -y -loglevel error -i " << shell_quote(wav_path)
        << " -c:a libmp3lame -b:a 32k -ac 1 " << shell_quote(output_path)
        << " >/dev/null 2>&1";
...
void tts_cancel_current() {
...

    {
        ...
        g_next_result_sequence = g_next_submit_sequence.load();
    }
...
void tts_speak_async(const std::string& text,
    ...
    std::string final_path;
    {
        std::lock_guard<std::mutex> lk(g_start_mutex);
        ++g_output_segment;
        final_path = std::string(TTS_OUTPUT_DIR) + "/result" +
    ...
    std::thread([text, generation, sequence, final_path, temporary_path]() {
        dl_acquire_slot();
        ...
        dispatch_segment_result(sequence,
            {generation, text, std::move(mp3), final_path, std::move(wav)});
        {
            std::lock_guard<std::mutex> lk(g_dl_mutex);
            --g_dl_pending;
            auto it = g_dl_pending_by_generation.find(generation);
