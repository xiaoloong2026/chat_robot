...
bool OpusAccumulator::feed(const std::vector<uint8_t>& data,
                            ...
                            uint8_t  seq) {
    if (state > 3) {
        const std::string error = "音频提问 STATE 超出 0..3";
        std::cerr << "[OPUS] " << error << "，丢弃当前提问" << std::endl;
        reset();
        last_error_ = error;
        return false;
    }
    if (state == 0) {
        if (data.empty()) return false;  // 安全忽略空 NULL/保活帧。
        const std::string error = "音频提问 STATE=0(NULL) 只允许空 CODE";
        std::cerr << "[OPUS] " << error << "，丢弃当前提问" << std::endl;
        reset();
        last_error_ = error;
        return false;
    ...
    // if (!active_) {
    //     last_error_ = "音频提问未由 STATE=1 开始";
    //     std::cerr << "[OPUS] " << last_error_ << "，忽略音频分片"
    //               << std::endl;
    //     return false;
    // }
    if (dsid == 0) {
        const std::string error = "音频提问 DSID 不能为 0";
        std::cerr << "[OPUS] " << error << "，丢弃当前提问" << std::endl;
        reset();
        last_error_ = error;
        return false;
    ...
    if (last_dsid_ == 0xFFFF) {
        const std::string error =
            "音频提问 DSID 已到 65535，协议未定义回绕";
        std::cerr << "[OPUS] " << error << "，丢弃当前提问" << std::endl;
        reset();
        last_error_ = error;
        return false;
    }
    const uint16_t expected = last_dsid_ == 0
        ? 1 : static_cast<uint16_t>(last_dsid_ + 1);
    if (dsid != expected) {
        const std::string error =
            "音频提问 DSID 不连续，期望 " + std::to_string(expected) +
            "，实际 " + std::to_string(dsid);
        std::cerr << "[OPUS] " << error << "，丢弃当前提问" << std::endl;
        reset();
        last_error_ = error;
        return false;
    ...
    static constexpr size_t MAX_OPUS_BYTES = 4 * 1024 * 1024;
    if (opus_buf_.size() + data.size() > MAX_OPUS_BYTES) {
        const std::string error = "音频提问累计数据超过 4 MiB";
        std::cerr << "[OPUS] " << error << "，丢弃当前提问" << std::endl;
        reset();
        last_error_ = error;
        return false;
    ...
    if (state == 3) {
        if (!opus_buf_.empty()) {
            ...
            if (!wav.empty() && on_decode_) {
                ...
                last_error_ = "OPUS 音频解码失败";
            }
            return true;
        }
        last_error_ = "STATE=3 时音频 CODE 为空";
        reset();
    }
...
std::string OpusAccumulator::take_error() {
